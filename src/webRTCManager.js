...
class WebRTCManager {
  constructor(localPeerId, stunServerAddress, uiConfig) {
    ...
    this.onPeerDisconnected = null; // (peerId: string) => {}


    this.onVideoMetadataReceived = null;
  ...
  _setupPeerConnectionEventHandlers(peerId, pc) {
    ...
    pc.ontrack = (event) => {
      ...
      if (event.receiver && kind === "video") {
        this._setupEncodedTransformForReceiver(peerId, event.receiver);
      }
    ...
    // Encoded Transform: parse [videoBytes | metadataBytes | metaLen] sent from Unity
  _setupEncodedTransformForReceiver(peerId, receiver) {
    if (!receiver || typeof receiver.createEncodedStreams !== "function") {
      console.warn("[EncodedTransform] receiver.createEncodedStreams not supported in this browser.");
      return;
    }
    ...
    const transformStream = new TransformStream({
      transform: (chunk, controller) => {
        ...
        if (metaLen + 1 > data.length) {
          console.warn("[EncodedTransform] Invalid metaLen", metaLen, "for length", data.length);
          controller.enqueue(chunk);
          return;
        }

        const videoLen = data.length - 1 - metaLen;
        if (videoLen <= 0) {
          console.warn("[EncodedTransform] Non-positive videoLen", videoLen);
          controller.enqueue(chunk);
          return;
        }

        const videoBytes    = data.subarray(0, videoLen);
        const metadataBytes = data.subarray(videoLen, videoLen + metaLen);

        let metadataText = null;
        try {
          metadataText = textDecoder.decode(metadataBytes);
        } catch (e) {
          console.error("[EncodedTransform] Failed to decode metadata:", e);
        }

        // Notify app if callback set
        if (this.onVideoMetadataReceived) {
          try {
            this.onVideoMetadataReceived(peerId, metadataText, metadataBytes);
          } catch (cbErr) {
            console.error("[EncodedTransform] Error in onVideoMetadataReceived:", cbErr);
          }
        }
